...
export class SkuProposalsService {
  ...
  async submit(id: string, userId: string) {
    ...
    const errorCount = existing.items.filter(
      (item: any) => item.validationStatus === 'ERROR'
    ).length;
  ...
  async importItems(id: string, parsedItems: ParsedSKU[]) {
    ...
    const categoryCodeMap = new Map(
      categories.map((c: any) => [c.code.toLowerCase(), c.id])
    );
  ...
  async validateItems(id: string) {
    ...
    const existingSKUCodes = new Set(existingSKUs.map((s: any) => s.skuCode));

    // Validate each item
    const validationResults = proposal.items.map((item: any, index: any) => {
      const errors: string[] = [];
    ...
    await this.prisma.$transaction(
      proposal.items.map((item: any, index: any) => {
    ...
    const warningCount = validationResults.filter((r: any) => r.status === 'warning').length;
    const errorCount = validationResults.filter((r: any) => r.status === 'error').length;
