...
export class WorkflowsService {
  ...
  async getApprovals(query: {
    ...
    const enrichedWorkflows = await Promise.all(
      workflows.map(async (workflow: any) => {
        ...
        const currentStep = workflow.steps.find(
          (s: any) => s.stepNumber === workflow.currentStep
        );
    ...
    const summary = {
      budget: allWorkflows.filter((w: any) => w.type === 'BUDGET_APPROVAL').length,
      otb: allWorkflows.filter((w: any) => w.type === 'OTB_APPROVAL').length,
      sku: allWorkflows.filter((w: any) => w.type === 'SKU_APPROVAL').length,
    ...
    if (action === 'REJECT') {
      ...
      if (isLastStep) {
        ...
        const nextStep = workflow.steps.find(
          (s: any) => s.stepNumber === workflow.currentStep + 1
        );
        if (nextStep) {
          await this.prisma.workflowStep.update({
