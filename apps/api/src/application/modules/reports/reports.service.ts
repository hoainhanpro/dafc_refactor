...
export class ReportsService {
  ...
  async getBudgetSummary(query?: {
    ...
    const summary = {
      ...
      totalValue: budgets.reduce((sum: any, b: any) => sum + Number(b.totalBudget), 0),
  ...
  async getOtbAnalysis(query?: {
    ...
    const summary = {
      ...
      totalOTBValue: otbPlans.reduce((sum: any, p: any) => sum + Number(p.totalOTBValue), 0),
      totalSKUs: otbPlans.reduce((sum: any, p: any) => sum + p.totalSKUCount, 0),
      byStatus: {} as Record<string, number>,
    };

    otbPlans.forEach((plan: any) => {
  ...
  async getSkuPerformance(query?: {
    ...
    const summary = {
      ...
      totalQuantity: skuItems.reduce((sum: any, s: any) => sum + s.orderQuantity, 0),
      totalValue: skuItems.reduce((sum: any, s: any) => sum + Number(s.orderValue || 0), 0),
      byCategory: {} as Record<string, { count: number; value: number }>,
      byValidationStatus: {} as Record<string, number>,
    };

    skuItems.forEach((sku: any) => {
