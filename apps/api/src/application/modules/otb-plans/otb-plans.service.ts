...
export class OtbPlansService {
  ...
  async submit(id: string, userId: string) {
    ...
    const totalPlannedAmount = existing.lineItems.reduce(
      (sum: any, item: any) => sum + Number(item.userBuyValue),
      0
    );
  ...
  async approve(id: string, userId: string, comments?: string) {
    ...
    if (existing.workflow) {
      const currentStep = existing.workflow.steps.find(
        (s: any) => s.stepNumber === existing.workflow!.currentStep
      );
      ...
      if (isLastStep) {
        ...
        const nextStep = existing.workflow.steps.find(
          (s: any) => s.stepNumber === existing.workflow!.currentStep + 1
        );
        if (nextStep) {
          await this.prisma.workflowStep.update({
  ...
  async getSizing(id: string) {
    ...
    const sizingMap = new Map(
      plan.sizingAnalysis.map((sa: any) => [`${sa.categoryId}-${sa.gender}`, sa])
    );

    const sizingData = plan.lineItems.map((item: any) => {
      ...
      return {
        ...
        sizing: sizing
          ? {
              id: sizing.id,
              sizeData: sizing.sizeData as Record<string, any>,
  ...
  async saveSizing(id: string, data: { categoryId: string; gender: string; sizeData: Record<string, any> }) {
    ...
    const total = Object.values(sizeData).reduce((sum: any, val: any) => sum + val, 0);
  ...
  async generateAIProposal(id: string, userId: string) {
    ...
    const historicalData = historicalPlans.map((hp: any) => ({
      season: hp.budget.season.code,
      totalBudget: Number(hp.budget.totalBudget),
      lineItems: hp.lineItems.map((li: any) => ({
    ...
    const proposals = categories.map((category: any) => {
      // Find historical data for this category
      const categoryHistory = historicalData.flatMap((hp: any) =>
        hp.lineItems.filter((li: any) => li.category === category.name)
      );

      // Calculate average percentage from history
      const avgPct = categoryHistory.length > 0
        ? categoryHistory.reduce((sum: any, li: any) => sum + Number(li.buyPct), 0) / categoryHistory.length
    ...
    return {
      proposals,
      overallConfidence: proposals.reduce((sum: any, p: any) => sum + p.confidence, 0) / proposals.length,
